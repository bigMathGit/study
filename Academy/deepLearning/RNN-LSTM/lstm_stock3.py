...
def lstm_stock(fname):
  ...
  df = df[df.Open != 0][['Open', 'High', 'Low', 'Close']]
  ...
  WINDOWSIZE = 60
  train_x, train_y = DATA.getSeriesData(train, WINDOWSIZE, elementdim=4)
  valid_x, valid_y = DATA.getSeriesData(validation, WINDOWSIZE, elementdim=4)
  test_x, test_y = DATA.getSeriesData(test, WINDOWSIZE, elementdim=4)
  ...
  lstm = LSTM.LSTM(4, WINDOWSIZE, 4, 4, 1, loss='square', opt='adam')
